...
export class TrabajadorComponent {
  ...
  guardarDatos($event: any) {
    ...
    this.nuevoTrabajador = {
      //Esto es equivalente a en java nuevoTrabajador = new Trabajador();
      nombre: '',
      apellidos: '',
      email: '',
      edad: 0,
  ...
  validarCampos() {
    if (
      this.nuevoTrabajador.nombre.trim().length > 3 ||
      this.nuevoTrabajador.apellidos.trim().length > 3 ||
      this.nuevoTrabajador.email.trim().length > 3 ||
      this.nuevoTrabajador.telefono.trim().length > 3 ||
      this.nuevoTrabajador.edad > 0
    ) {
